...
def full_prophecy(
    ...
    if not token_result.get('token_score'):
        token_result['token_score'] = 20

    # Extract sub-scores from prophecy_engine's result
    # prophecy_engine now returns token_score, deployer_score, promoter_score
    raw_token_score    = token_result.get('token_score',    20)
    raw_deployer_score = token_result.get('deployer_score', 20)
    raw_promoter_score = token_result.get('promoter_score', 20)
